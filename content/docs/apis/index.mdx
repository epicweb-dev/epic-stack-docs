---
title: APIs in the Epic Stack
date: 2023-11-04
description: How the Epic Stack enables building APIs with resource routes and full stack components.
categories:
  - api
meta:
  keywords:
...
Remix routes have the ability to handle both backend code and UI code in the same file. Remix `loader`s and `action`s are backend code that's tightly coupled to the UI code for that route. 
